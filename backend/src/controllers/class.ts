...
export const createClass = async(req: Request, res: Response) => {
    try{
        ...
        await logActivity({
            ...
            action: `Created class ${name} for academic year ${academicYear}`,
        });
...
export const updateClass = async(req: Request, res: Response) => {
    try{
        ...
        if(existingClass){
            ...
            await logActivity({
                userId: (req as any).user.id,
                action: `Updated class ${updatedClass.name} for academic year ${updatedClass.academicYear}`,
            });
...
export const deleteClass = async(req: Request, res: Response) => {
    try{
        ...
        await logActivity({
            userId,
            action: `Deleted class ${deletedClass?.name} for academic year ${deletedClass?.academicYear}`,
        });
        if(!deletedClass){
            return res.status(404).json({ message: "Class not found" });
        }
